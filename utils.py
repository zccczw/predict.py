...
def normalize_data(data, scaler=None):
    ...
    if scaler is None:
        scaler = MinMaxScaler()  #找出第几个元祖的值最大
        scaler.fit(data)
    data = scaler.transform(data)  #对元祖进行归一化
...
def plot_losses(losses, save_path="", plot=True):
    ...
    # plt.plot(losses["train_reconrecon"], label="Recon loss")
...
def adjust_anomaly_scores(scores, dataset, is_train, lookback):
    """
   MSL 和 SMAP 的方法，其中通道已连接作为预处理的一部分
