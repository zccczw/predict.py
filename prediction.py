...
class Predictor:
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        print(f"POT method 耗时:\n {time.time() - start}")

        start = time.time()
        p_eval_mom = pot_eval_mom(train_anomaly_scores, test_anomaly_scores, true_anomalies,
                                  q=self.q, level=self.level, dynamic=self.dynamic_pot)
        print(f"POT_MOM method 耗时:\n {time.time() - start}")

        start = time.time()
        interFusion = get_best_f1(test_anomaly_scores, true_anomalies)
        print(f"InterFusion method 耗时:\n {time.time() - start}")

        bf_eval = {}
        if true_anomalies is not None:
            start = time.time()
            bf_eval = bf_search(test_anomaly_scores, true_anomalies, start=0.3, end=1, step_num=1000, verbose=False)
            print(f"best f1 score search method 耗时:\n {time.time() - start}")

        print(f"Results using epsilon method:\n {e_eval}")
        print(f"Results using POT method:\n {p_eval}")
        print(f"Results using POT-MoM method:\n {p_eval_mom}")
        print(f"Results using InterFusion method:\n {interFusion}")
        ...
        for k, v in bf_eval.items():
            bf_eval[k] = float(v)
        for k, v in interFusion.items():
            interFusion[k] = float(v)
        for k, v in p_eval_mom.items():
            p_eval_mom[k] = float(v)

        # Save
        summary = {"level": self.level, "q": self.q, "epsilon_result": e_eval, "POT_result": p_eval,
                   "POT-MoM_result": p_eval_mom, "InterFusion_result": interFusion, "bf_result": bf_eval}
