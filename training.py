...
class Trainer:
    ...
    :param dload: 要转储模型的下载目录
    :param log_dir: SummaryWriter 日志写入的目录
    ...
    def fit(self, train_loader, val_loader=None, timeInfo=None):
        """Train model for self.n_epochs.
        存储在 self.losses 中的训练和验证（如果提供了验证加载程序）损失
        ...
        for epoch in range(self.n_epochs):
            ...
            for x, y in train_loader:
                ...
                if preds.ndim == 3:
                    preds = preds.squeeze(1)    #squeeze函数用来降维的
                if y.ndim == 3:
                    y = y.squeeze(1)
            ...
            timeInfo[f"epoch_{epoch}_time"] = epoch_time
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():  #x[128,60,25]
        ...
        return forecast_loss, 0, total_loss
