...
class SPOT:
    ...
    def initialize(self, level=0.98, min_extrema=False, verbose=True):
        ...
        S = np.sort(self.init_data)  # 我们对 X 进行排序以获得经验分位数
        self.init_threshold = S[int(level * n_init)]  # 整个算法的 t 是固定的
    ...
    #     """
    #     用 Grimshaw 的技巧计算 GPD 参数估计
    #
    #     Parameters
    #     ----------
    #     epsilon : float
    #             要执行的数值参数(default : 1e-8)
    #     n_points : int
    #             最大似然的最大候选数 (default : 10)
    #     Returns
    #     ----------
    #     gamma_best,sigma_best,ll_best
    #            gamma 估计、sigma 估计和 相应的对数似然
    #     """
    #
    #     def u(s):
...
“threshold”包含极端分位数，“alarms”包含触发警报的值的索引
        ...
        for i in tqdm.tqdm(range(self.data.size)):
            ...
            th.append(self.extreme_quantile)  # 阈值记录
...
class biSPOT:
    """
    此类允许在单变量数据集（上限和下限）上运行 biSPOT 算法
...
class bidSPOT:
    """
    此类允许在单变量数据集（上限和下限）上运行 DSPOT 算法
